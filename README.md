...
本仓库包含了期终试卷的第八题（强化学习）和第九题（回归预测）的解决方案代码。
...
*   **图像处理**: 程序首先读取 `maze.jpg`，将其转换为网格环境（Grid World）。
*   **目标选择**:
    *   默认情况下，程序会自动寻找距离起点最远的有效点作为终点。
    *   程序启动后，您可以选择手动输入终点坐标。
*   **训练过程**: 智能体会进行 10,000 次回合（Episode）的训练。
*   **结果输出**:
    *   控制台输出训练进度、最佳路径长度。
...
该模块构建了一个基于 Gradient Boosting 的回归模型，用于对给定数据进行预测和评估。
...
*   **模型管道**:
    *   预处理：数值特征标准化 (StandardScaler)，分类特征独热编码 (OneHotEncoder)。
